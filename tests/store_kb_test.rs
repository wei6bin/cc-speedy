use cc_speedy::store::{get_setting, load_learnings, save_learnings, set_setting, LearningPoint};
...
fn test_save_and_load_learnings() {
    ...
    let points = vec![
        LearningPoint {
            ...
            point: "chose SQLite".to_string(),
        },
        LearningPoint {
            category: "lessons_gotchas".to_string(),
            point: "watch out for lock".to_string(),
        },
    ];
...
fn test_save_learnings_appends_not_replaces() {
    let conn = make_in_memory_db();
    let first = vec![LearningPoint {
        category: "lessons_gotchas".to_string(),
        point: "first lesson".to_string(),
    }];
    let second = vec![LearningPoint {
        category: "lessons_gotchas".to_string(),
        point: "second lesson".to_string(),
    }];
...
fn test_get_and_set_setting() {
    ...
    assert_eq!(
        get_setting(&conn, "obsidian_kb_path").as_deref(),
        Some("/tmp/vault")
    );
}
